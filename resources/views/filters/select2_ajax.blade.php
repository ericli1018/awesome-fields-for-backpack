...
@push('crud_list_scripts')
	<!-- include select2 js-->
    <script src="{{ basset(base_path('vendor/select2/select2/dist/js/select2.full.min.js')) }}"></script>
    @if (app()->getLocale() !== 'en')
    <script src="{{ basset(base_path('vendor/select2/select2/dist/js/i18n/' . str_replace('_', '-', app()->getLocale()) . '.js')) }}"></script>
    @endif
    @bassetBlock('backpack/crud/filters/select2_ajax.js')
    <script>
        jQuery(document).ready(function($) {
            // trigger select2 for each untriggered select2 box
            $('li[filter-type=select2_ajax]').not('[data-filter-enabled]').each(function () {
                ...
                $(_self).find("select").each(function () {
                    ...
                    var filterValues = $(this).attr('data-filter-values');
