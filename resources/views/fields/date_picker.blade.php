...
@include('crud::fields.inc.wrapper_start')
    <input type="hidden" class="form-control" name="{{ $field['name'] }}" value="{{ $field['value'] }}"><label>{!! $field['label'] !!}</label>
...
@push('crud_fields_scripts')
    <!-- include date_picker js -->
    <script src="{{ basset('https://cdn.jsdelivr.net/npm/bootstrap-datepicker@1.10.0/dist/js/bootstrap-datepicker.min.js') }}" crossorigin="anonymous"></script>
    @if (app()->getLocale() !== 'en')
    <script src="{{ basset('https://cdn.jsdelivr.net/npm/bootstrap-datepicker@1.10.0/dist/locales/bootstrap-datepicker.' . str_replace('_', '-', app()->getLocale()) . '.min.js') }}" crossorigin="anonymous"></script>
    @endif
    @bassetBlock('backpack/crud/fields/date_picker.js')
    <script>
        if (jQuery.ui) {
