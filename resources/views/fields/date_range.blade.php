...
@php
    ...
    $field_names = preg_split ("/\,/", $field['name']);
    if (isset($entry)) {
        $start_value = formatDate($entry, $field_names[0]);
        $end_value = formatDate($entry, $field_names[1]);
    }
...
@include('crud::fields.inc.wrapper_start')
    <input class="datepicker-range-start" type="hidden" name="{{ $field_names[0] }}" value="{{ old(square_brackets_to_dots($field['name'][0])) ?? $start_value ?? $start_default ?? '' }}">
    <input class="datepicker-range-end" type="hidden" name="{{ $field_names[1] }}" value="{{ old(square_brackets_to_dots($field['name'][1])) ?? $end_value ?? $end_default ?? '' }}">
...
@push('crud_fields_styles')
    <!-- include date_range css -->
    <link href="{{ basset('https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.css') }}" rel="stylesheet" type="text/css" />
@endpush

{{-- CUSTOM JS --}}
@push('crud_fields_scripts')
    <!-- include date_range js -->
    <script src="{{ basset('https://cdn.jsdelivr.net/npm/moment@2.30.1/moment.min.js') }}" crossorigin="anonymous"></script>
    @if (app()->getLocale() !== 'en')
    <script src="{{ basset('https://cdn.jsdelivr.net/npm/moment@2.30.1/locale/' . str_replace('_', '-', strtolower(app()->getLocale())) . '.js') }}" crossorigin="anonymous"></script>
    @endif
    <script src="{{ basset('https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js') }}" crossorigin="anonymous"></script>
    @bassetBlock('backpack/crud/fields/date_range.js')
    <script>
